...
// One L/C/R row: label, "count (pct%)", and a thin bar.
const CountRow: React.FC<CountRowProps> = ({ label, count, percent, color }) => (
  <div className="flex items-center gap-3">
    ...
    <span className="w-16 text-body-small font-mono text-text-primary">
      {count} ({percent}%)
    </span>
...
export const SourceBreakdownPanel: React.FC<SourceBreakdownPanelProps> = ({
  ...
  return (
    <SidebarCard title="Source Breakdown" className={className}>
      <p className="text-body-small text-text-secondary mb-4">{sourcesCount} Total Sources</p>
